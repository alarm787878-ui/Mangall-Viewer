...
async function notifyCustomSiteChange() {
  if (!browserApi?.sendRuntimeMessage) return true;

  try {
    const response = await browserApi.sendRuntimeMessage({
...
async function notifyViewerShortcutSettings() {
  if (!extensionApi?.tabs?.query || !extensionApi?.tabs?.sendMessage) return;
...
async function renderCustomSites() {
  ...
  for (const site of sites) {
    ...
    deleteButton.className = "danger_btn delete-btn";
